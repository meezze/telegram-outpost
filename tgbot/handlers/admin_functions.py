...
from utils.functions import clear_html
from utils.db_api import get_userx, update_userx, get_refillx, get_purchasex, upload_text

# Настройте PayPal API
...
# Создайте функцию для создания PayPal платежа
def create_paypal_payment(amount, currency="MDL", description="Payment"):
...
# Вместо вызова функции оплаты через Qiwi, вызовите созданную функцию create_paypal_payment с необходимыми параметрами
amount = 10  # Сумма платежа
currency = "MDL"  # Валюта платежа
description = "Оплата товара"  # Описание платежа
user_id = 123456789  # ID пользователя

payment = create_paypal_payment(amount, currency, description)
...
if payment:
    ...
    # Отправить пользователю ссылку для подтверждения платежа
    asyncio.create_task(bot.send_message(user_id, f"Пожалуйста, перейдите по ссылке для оплаты: {approval_url}"))
    ...
    def save_payment(payment_id, user_id, amount, currency, description, status):
        conn = setup_database()
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO payments (payment_id, user_id, amount, currency, description, status)
            VALUES (?,INSERT INTO payments (payment_id, user_id, amount, currency, description, status)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (payment_id, user_id, amount, currency, description, status))
        conn.commit()
        conn.close()
    ...
    if payment:
        ...
        save_payment(payment_id, user_id, amount, currency, description, status)
    else:
        # Обработка ошибки платежа
        error_message = f"Произошла ошибка при создании платежа: {payment.error}"
        print(error_message)
        asyncio.create_task(bot.send_message(user_id, error_message))
...
# Прием номера чека от администратора
@dp.message_handler(IsAdmin(), Text(), state="here_receipt")
async def functions_receipt_get(message: Message, state: FSMContext):
    receipt_number = message.text.strip()

    # Получить информацию о чеке из вашей базы данных
    purchase_info = get_purchase_info(receipt_number)

    if purchase_info:
        # Отправить информацию о чеке администратору
        await message.answer(
            f"<b>🧾 Чек: {purchase_info['purchase_receipt']}</b>\n"
            f"👤 Пользователь: {purchase_info['user_id']}\n"
            f"🎁 Товар: {purchase_info['purchase_position_name']} | {purchase_info['purchase_count']}шт | {purchase_info['purchase_price']}₽\n"
            f"🕰 Дата покупки: {purchase_info['purchase_date']}\n"
        )
    else:
        await message.answer("❌ Не удалось найти чек с указанным номером.")
