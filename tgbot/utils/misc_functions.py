...
async def check_update():
    r = requests.get('https://t.me/TGinfo_bot')

    soup_parse = BeautifulSoup(r.text, 'lxml')

    try:
        get_bot_info = soup_parse.select("p[class$='CDt4Ke zfr3Q']")[0].text.split("=")
    except IndexError:
        bot_logger.exception("Error checking bot update")
        print("Error checking bot update")
        return

    if get_bot_info[1][:3] != BOT_VERSION:
        with open("tgbot/data/config.py", "a") as file:
            file.write(f"\n\nBOT_DESCRIPTION = '{get_bot_info[0]}'\n")
            file.write(f"BOT_VERSION = '{get_bot_info[1][:3]}'")
